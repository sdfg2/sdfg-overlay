...
SRC_URI="https://www.process-one.net/downloads/downloads-action.php?file=/ejabberd/14.12/ejabberd-14.12.tgz -> ${P}.tar.gz
...
IUSE="captcha debug ldap odbc pam postgres +web zlib ${EJABBERD_MODULES}"
...
pkg_postinst() {
	if [[ -z ${REPLACING_VERSIONS} ]] ; then
		...
		elog '1) Add output of `hostname -f` to /etc/jabber/ejabberd.yml line TODO'
		elog '   {hosts, ["localhost", "thehost"]}.'
		elog '2) Add an admin user to /etc/jabber/ejabberd.yml line TODO'
